...
extension PXPaymentMethodSelector {
    func startTracking(then: @escaping (() -> Void)) {
        ...
        } else if let checkoutPreference = viewModel?.checkoutPreference {
            properties["checkout_preference"] = getCheckoutPrefForTracking(checkoutPreference: checkoutPreference)
        }
        ...
        properties["split_enabled"] = false
