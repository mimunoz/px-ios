...
final class NewPaymentMethodSmallCardView: UIView {
    ...
    private struct Sizes {
        static let circleIconWidth: CGFloat = 35
        static let circleIconHeight: CGFloat = 35
...
extension NewPaymentMethodSmallCardView: ViewConfiguration {
    func buildHierarchy() {
        addSubviews(views: [containerStack])
        containerStack.addArrangedSubview(detailContainerStackView)
        textsContainerView.addSubviews(views: [titleLabel, subtitleLabel])
        detailContainerStackView.addArrangedSubviews(views: [circleIcon, textsContainerView])
    }

    func setupConstraints() {
        NSLayoutConstraint.activate([
            titleLabel.bottomAnchor.constraint(equalTo: subtitleLabel.topAnchor, constant: -2),
            titleLabel.topAnchor.constraint(equalTo: textsContainerView.topAnchor),
            titleLabel.leadingAnchor.constraint(equalTo: textsContainerView.leadingAnchor),
            titleLabel.trailingAnchor.constraint(equalTo: textsContainerView.trailingAnchor),

            subtitleLabel.bottomAnchor.constraint(equalTo: textsContainerView.bottomAnchor),
            subtitleLabel.leadingAnchor.constraint(equalTo: textsContainerView.leadingAnchor),
            subtitleLabel.trailingAnchor.constraint(equalTo: textsContainerView.trailingAnchor),
